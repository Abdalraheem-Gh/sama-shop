...
const calcPrice=(items:CartItem[])=>{
    ...
    taxPrice=round2(0.25*itemsPrice),
...
export async function addItemToCart(data:CartItem) {
    try {
        ...
        const userId=session?.user?.id?(session.user.id as string):undefined;// لتجنب الاخطاء لان عادي انو يكون المستخدم غير مسجل دخول
